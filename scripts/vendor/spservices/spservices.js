...
(function ($) {
	...
	// Version info
	var VERSION					= "2013.02";			// TODO: Update version
	...
	// Global variables
	var SPServicesContext = new SPServicesContextGet();	// Variable to hold the current context as we figure it out
	...
	$.fn.SPServices = function(options) {
		...
		switch(opt.operation) {
			...
			case "GetListItemChanges":
				addToPayload(opt, ["listName", ["viewFields", "CAMLViewFields"], "since", {name: "contains", sendNull: false}]);
				break;
		...
		var cachedPromise;
		if(opt.cacheXML) {
			cachedPromise = promisesCache[ajaxURL + msg];
		}
		...
		if(typeof cachedPromise === "undefined") {
			...
			promisesCache[ajaxURL + msg] = $.ajax({
			...
			// Return the promise
			return promisesCache[ajaxURL + msg];
				...
				$().SPServices({
					...
					completefunc: function(xData) {
						...
						$(xData.responseXML).find("User").each(function() {
							...
							$.each(this.attributes, function(attrib){ 
								var key = attrib.name; 
...
////// PRIVATE FUNCTIONS ////////
	...
	function addToPayload(opt, paramArray) {
		...
		for(i=0; i < paramArray.length; i++) {
			...
			} else if(!Array.isArray(paramArray[i])) {
				SOAPEnvelope.payload += ((opt[paramArray[i].name] === undefined) || ((opt[paramArray[i].name].length === 0) && (paramArray[i].sendNull === false))) ? "" : wrapNode(paramArray[i].name, opt[paramArray[i].name]);
